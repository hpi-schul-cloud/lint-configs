...
<p align="center">
  ...
  <a href="https://github.com/schul-cloud/lint-configs/blob/master/LICENSE.md"><img src="https://img.shields.io/npm/l/@schul-cloud/cli.svg?style=flat-square" alt="MIT License"></a>
  <a href="https://www.npmjs.com/package/@schul-cloud/cli"><img src="https://img.shields.io/npm/v/@schul-cloud/cli.svg" alt="Version"></a>
</p>
...
* [@schul-cloud/babel-preset](./packages/babel-preset/README.md) - This package contains shareable babel.js configuration used by the applications created with @schul-cloud/cli.

* [@schul-cloud/cli](./packages/cli/README.md) - A next-generation CLI to quickly scaffold pre-configured yet on-demand configurable React applications.
...
* [@schul-cloud/scripts](./packages/scripts/README.md) - This package contains automation task used by the applications created with @schul-cloud/cli.

* [@schul-cloud/typescript-config](./packages/typescript-config/README.md) - This package contains shareable TypeScript compiler configuration used by the applications created with @schul-cloud/cli.

* [@schul-cloud/webpack-config](./packages/webpack-config/README.md) - This package contains shareable webpack configuration used by the applications created with @schul-cloud/cli.

* [@schul-cloud/helpers](./packages/helpers/README.md) - This package contains common helper utilities for `schul-cloud` core.

# Getting Started

## Installation

The first step is to install `@schul-cloud/cli` to quickly scaffold an application.

Using Yarn:

```shell
yarn global add @schul-cloud/cli
```

Using npm:

```shell
npm install @schul-cloud/cli -g
```
...
```shell
schul-cloud new my-demo-app
```

If you want to look into the application that will be created by this command, take a look at [schul-cloud-starter-javascript](https://github.com/schul-cloud/lint-configs-starter-javascript).
...
```shell
schul-cloud new my-demo-app --typescript
```

If you want to look into the application that will be created by this command, take a look at [schul-cloud-starter-typescript](https://github.com/schul-cloud/lint-configs-starter-typescript).

This will generate a new application for you in current directory.

## Configuration Options

Apps created with `@schul-cloud/cli` are built with sensible defaults yet very extensible at the core.

### schul-cloudrc.json

In order to configure certain behavior of development workflow, you can play with the options in `schul-cloudrc.json`. Here's how the file looks like in a newly scaffolded app:
...
As `schul-cloud` uses Babel to convert TypeScript to JavaScript, so Babel just strips away types without any type-checking. This usually results in faster incremental builds but takes away the important aspect of TypeScript, type-checking. And the only way left is that you can check the soundness of types via your IDE intellisense features. This is not helpful always. There are two ways to type-check your TypeScript code in `schul-cloud`.
  * You can turn on `typeCheck` flag in `schul-cloudrc.json` by turning its value to `true`. This will use `fork-ts-checker-webpack-plugin` for type-checking in every compilation. Note that this is only available when `typescript` flag value is `true` and won't do anything for JavaScript-React projects. This might leads to slower incremental builds.

  * Second option is to run the type-checking script that's exposed by `schul-cloud`. You can run it as follows:

    ```shell
    schul-cloud type-check
    ```
...
- **title** (string: `React App | schul-cloud`)  
...
Although, apps created with `@schul-cloud/cli` does include sensible defaults when it comes to configurations. But at its heart, every tool is super configurable. Check more about configuring individual tools in pipelines in [Configuring other tools](#configuring-other-tools) section.

There are some opinions, however:

* `schul-cloudrc.json` file must exist at the root of project directory.
...
Note that the apps scaffolded with `@schul-cloud/cli` already follow the opinions described above. However, these rules will be validated every time you run a standard `@schul-cloud/cli` command and if you try to violate them, you'll be prompted about what you're doing wrong and what you should do to fix.
...
<!-- prettier-ignore -->
<table><tr><td align="center"><a href="https://www.shahzaibkhalid.com"><img src="https://avatars3.githubusercontent.com/u/18225515?v=4" width="100px;" alt="Shahzaib Khalid"/><br /><sub><b>Shahzaib Khalid</b></sub></a><br /><a href="#question-shahzaibkhalid" title="Answering Questions">💬</a> <a href="https://github.com/schul-cloud/lint-configs/issues?q=author%3Ashahzaibkhalid" title="Bug reports">🐛</a> <a href="https://github.com/schul-cloud/lint-configs/commits?author=shahzaibkhalid" title="Code">💻</a> <a href="#design-shahzaibkhalid" title="Design">🎨</a> <a href="https://github.com/schul-cloud/lint-configs/commits?author=shahzaibkhalid" title="Documentation">📖</a> <a href="#example-shahzaibkhalid" title="Examples">💡</a> <a href="#fundingFinding-shahzaibkhalid" title="Funding Finding">🔍</a> <a href="#ideas-shahzaibkhalid" title="Ideas, Planning, & Feedback">🤔</a> <a href="#maintenance-shahzaibkhalid" title="Maintenance">🚧</a> <a href="#projectManagement-shahzaibkhalid" title="Project Management">📆</a> <a href="#review-shahzaibkhalid" title="Reviewed Pull Requests">👀</a> <a href="#tool-shahzaibkhalid" title="Tools">🔧</a></td></tr></table>
...
`schul-cloud` is released under the [MIT License](./LICENSE.md).
